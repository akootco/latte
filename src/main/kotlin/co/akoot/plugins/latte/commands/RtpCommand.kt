...
import co.akoot.plugins.bluefox.api.Prices
...
class RtpCommand(plugin: Latte) : CatCommand(plugin, "rtp") {
    val invoice = Prices.Commands.rtp.invoice("a teleport to a random location")
    ...
    init {
        ...
        then(int("X radius", 0) {
            ...
            getSender(it).sendMessage(
                Kolor.TEXT("Teleported ") + player.displayName()
                    .color(Kolor.PLAYER.get()) + Kolor.TEXT(" to ") + location.text + "."
            )
            true
        })))))
    }
    ...
    fun cooldownCheck(player: Player): Boolean? {
        ...
        if (cooldown > now) {
            ...
            player.sendMessage(
                Kolor.WARNING("You must wait ") + Kolor.WARNING.number(
                    String.format(
                        "%.2f seconds",
                        timeLeft / 1000.0
                    )
                ) + Kolor.WARNING(" before using /$id again.")
            )
            return null
        }
        return true
    ...
    fun rtp(
        ...
        player.rtpCooldown = System.currentTimeMillis() + (plugin.settings.getInt("rtp.cooldown.min")
            ?: 10000) + (player.server.onlinePlayers.size * (plugin.settings.getInt("rtp.cooldown.per_player") ?: 1000))
